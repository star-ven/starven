...
export function readAuthenticatedIdentity(headers) {
  ...
  if (cloudflareEmail) {
    return {
      ...
      fullName: null,
      provider: /** @type {const} */ ("cloudflare-access"),
    };
  }
  return null;
}

export function configuredAdminIdentifier(provider, environment) {
  return provider === "cloudflare-access"
    ? environment.ADMIN_CONTACT_EMAIL
    : environment.ADMIN_USER_ID;
}
